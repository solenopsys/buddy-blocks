...
pub const BlockMetadata = struct {
    ...
    pub fn decode(data: []const u8) !BlockMetadata {
        ...
        return .{
            .block_size = try BlockSize.fromBytes(size_bytes),
...
/// Helper to create free list key: "4k_123" -> block of size 4k with number 123
pub fn makeFreeListKey(block_size: BlockSize, block_num: u64, buffer: []u8) ![]const u8 {
    return std.fmt.bufPrint(buffer, "{s}_{d}", .{ block_size.toString(), block_num });
}

/// Helper to parse free list key: "4k_123" -> {size: 4k, block_num: 123}
