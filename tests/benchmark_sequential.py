...
for i, block in enumerate(blocks):
    try:
        response = requests.put(f"{SERVER_URL}/block/", data=block, timeout=5)
...
for i, expected_hash in enumerate(hashes):
    try:
        response = requests.get(f"{SERVER_URL}/block/{expected_hash}", timeout=5)
        if response.status_code == 200:
...
for i, expected_hash in enumerate(hashes):
    try:
        response = requests.delete(f"{SERVER_URL}/block/{expected_hash}", timeout=5)
        if response.status_code == 200:
            delete_success += 1
        else:
            delete_failed += 1
